...
def plot_single_timestep(filepath, output_dir=None, show=True):
    ...
    fig.suptitle(f'IBPM Raw Data - Step {timestep:05d} (t = {time:.2f})',
...
def plot_force_coefficients(force_file, output_file=None, show=True):
    ...
    ax.axhline(y=fx[50:].mean(), color='r', linestyle='--', linewidth=1.5,
               label=f'Mean (t>1.0): {fx[50:].mean():.4f}')
...
def main():
    ...
    parser.add_argument('--data-dir', type=str, default='/workspace/data/ibpm_full',
                        help='Directory containing IBPM output files')
    parser.add_argument('--output-dir', type=str, default='/workspace/data/ibpm_plots',
                        help='Directory to save plots')
    parser.add_argument('--timestep', type=int, nargs='+', default=None,
                        help='Specific timestep(s) to plot')
    parser.add_argument('--evolution', action='store_true',
                        help='Plot flow evolution at multiple timesteps')
    ...
    if args.force:
        ...
        if force_file.exists():
            plot_force_coefficients(force_file,
                                   output_file=output_dir / 'force_coefficients.png',
                                   show=show)
        else:
